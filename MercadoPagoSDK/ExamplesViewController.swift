...
class ExamplesViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        case 0:
            self.showViewController(MercadoPago.startPaymentMethodsViewController(ExamplesUtils.MERCHANT_PUBLIC_KEY, supportedPaymentTypes: ["credit_card", "debit_card", "prepaid_card"], callback: { (paymentMethod: PaymentMethod) -> Void in
                self.showViewController(ExamplesUtils.startCardActivity(ExamplesUtils.MERCHANT_PUBLIC_KEY, paymentMethod: paymentMethod, callback: {(token: Token?) -> Void in
                    self.createPayment(token, paymentMethod: paymentMethod, installments: 1, cardIssuerId: nil, discount: nil)
        ...
        case 2:
            self.showViewController(ExamplesUtils.startAdvancedVaultActivity(ExamplesUtils.MERCHANT_PUBLIC_KEY, merchantBaseUrl: ExamplesUtils.MERCHANT_MOCK_BASE_URL, merchantGetCustomerUri: ExamplesUtils.MERCHANT_MOCK_GET_CUSTOMER_URI, merchantAccessToken: ExamplesUtils.MERCHANT_ACCESS_TOKEN, amount: ExamplesUtils.AMOUNT, supportedPaymentTypes: ["credit_card", "debit_card", "prepaid_card"], callback: {(paymentMethod: PaymentMethod, token: Token?, cardIssuerId: Int?, installments: Int?) -> Void in
                    self.createPayment(token, paymentMethod: paymentMethod, installments: installments, cardIssuerId: cardIssuerId, discount: nil)
            }), sender: self)
        case 3:
            // TODO: SHOW LOADING
            MerchantServer.getDiscount(ExamplesUtils.MERCHANT_MOCK_BASE_URL, merchantGetDiscountUri: ExamplesUtils.MERCHANT_MOCK_GET_DISCOUNT_URI, merchantAccessToken: ExamplesUtils.MERCHANT_ACCESS_TOKEN, itemId: ExamplesUtils.ITEM_ID, itemQuantity: ExamplesUtils.ITEM_QUANTITY, success: {(discount: Discount?) -> Void in
                // TODO: HIDE LOADING
                self.showViewController(ExamplesUtils.startFinalVaultActivity(ExamplesUtils.MERCHANT_PUBLIC_KEY, merchantBaseUrl: ExamplesUtils.MERCHANT_MOCK_BASE_URL, merchantGetCustomerUri: ExamplesUtils.MERCHANT_MOCK_GET_CUSTOMER_URI, merchantAccessToken: ExamplesUtils.MERCHANT_ACCESS_TOKEN, amount: ExamplesUtils.AMOUNT, supportedPaymentTypes: ["credit_card", "debit_card", "prepaid_card", "ticket", "atm"], discount: discount, callback: {(paymentMethod: PaymentMethod, token: Token?, cardIssuerId: Int?, installments: Int?) -> Void in
                    self.createPayment(token, paymentMethod: paymentMethod, installments: installments, cardIssuerId: cardIssuerId, discount: discount)
                }), sender: self)
                }, failure: nil)
        case 4:
            // TODO: SHOW LOADING
    ...
    func createPayment(token: Token?, paymentMethod: PaymentMethod, installments: Int?, cardIssuerId: Int?, discount: Discount?) {
        if token != nil {
            ExamplesUtils.createPayment(token!.id, installments: installments, cardIssuerId: cardIssuerId, paymentMethod: paymentMethod, discount: discount, callback: { (payment: Payment) -> Void in
